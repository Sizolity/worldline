...
// cmd/rpg-cli) can construct structured.Agent[Params].
type Params struct {
...
// satisfied by structured.Agent[Params] and by package-local mocks in
// tests.
type Agent = structured.Agent[Params]

// Resolver wraps a structured.Agent[Params] with the persona-aware
...
// NewResolver constructs a Resolver. agent is required. persona may be
// nil; when nil the embedded default intent persona (see
// default_persona.go) is used so the Resolver works in legacy /
// test paths that have no mod.Style loaded.
func NewResolver(agent Agent, persona *mod.Document) (*Resolver, error) {
	if agent == nil {
		return nil, fmt.Errorf("intent: agent is required")
	}
	if persona == nil {
		persona = defaultIntentPersona()
	}
...
func (r *Resolver) Resolve(ctx context.Context, rawInput string, choices role.ActionChoices, recentNarrative string) (Params, error) {
	...
	messages := []chat.Message{
		chat.SystemMessage(r.SystemPrompt()),
		chat.UserMessage(buildUserPrompt(rawInput, choices, recentNarrative)),
	}
