...
// mockLoreAgent is a configurable structured.Agent[LoreDraft] for LoreParser tests.
...
func (m *mockLoreAgent) Call(_ context.Context, _ []chat.Message) (LoreDraft, error) {
	m.called = true
	if m.err != nil {
...
func TestLoreParser_ParseSuccess(t *testing.T) {
	...
	parser := NewLoreParser(&mockLoreAgent{payload: payload})
...
func TestLoreParser_EmptyContentReturnsError(t *testing.T) {
	parser := NewLoreParser(&mockLoreAgent{err: errors.New("empty content")})
...
func TestLoreParser_GenerateError(t *testing.T) {
	parser := NewLoreParser(&mockLoreAgent{err: errors.New("boom")})
	_, err := parser.Parse(context.Background(), ingest.SourceDocument{ID: "doc-1", Text: "一些叙事。"})
	if err == nil {
