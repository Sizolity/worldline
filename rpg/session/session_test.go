...
func (m *mockGM) Tools(tc *tools.ToolContext) ([]agenttool.Tool, error) {
	return tools.NewInvokableTools(tc), nil
}
	...
	go func() {
		...
		if len(req.Tools) > 0 {
			for _, t := range req.Tools {
				if t.Info().Name == "roll" {
					_, _ = t.Invoke(context.Background(), `{"sides":20,"count":1,"modifier":2}`)
					break
				}
			}
		}
		...
		doneCh <- beat.Result{Narrative: narrative}
	}()
...
func (m *mockExploreBeatAgent) Run(_ context.Context, req beat.Request) *beat.Stream {
	narrativeCh := make(chan string, 1)
	doneCh := make(chan beat.Result, 1)
	...
	go func() {
		...
		if len(req.Tools) > 0 {
			for _, t := range req.Tools {
				if t.Info().Name == "explore_knowledge" {
					_, _ = t.Invoke(context.Background(), `{"target_id":"loc-cavern","level":"explored"}`)
					break
				}
			}
		}
