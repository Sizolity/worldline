...
// Lorekeeper, combining the mod-supplied persona document with the
// Go-hardcoded JSON schema constraints. The persona is always non-nil
// because (*Narrator).New rejects styles that omit it; callers must
// construct the Narrator through New to satisfy that contract.
func (n *Narrator) LorekeeperPrompt() string {
	if n.lorekeeperPersona == nil {
		panic("narrator: lorekeeper persona is nil — construct via narrator.New(suggestAgent, narrator.WithStyle(style))")
	}
	return mod.RenderAuxiliaryPrompt(n.lorekeeperPersona, lorekeeperEngineConstraints)
}
...
// NewLoreParser constructs a LoreParser bound to the given typed agent.
// The Narrator carries the lorekeeper persona (loaded from a mod.Style
// via narrator.WithStyle) and the engine compliance trailer; n must be
// non-nil and successfully constructed via New.
func NewLoreParser(agent typed.Agent[LoreDraft], n *Narrator) *LoreParser {
	if n == nil {
		panic("narrator: NewLoreParser requires a non-nil *Narrator (persona source)")
	}
	return &LoreParser{agent: agent, prompt: n.LorekeeperPrompt()}
}
