...
// action suggester, combining the mod-supplied persona doc with the
// Go-hardcoded engine constraints. The persona is always non-nil
...
func (n *Narrator) SuggesterPrompt() string {
	if n.suggesterPersona == nil {
		panic("narrator: suggester persona is nil — construct via narrator.New(suggestAgent, narrator.WithStyle(style))")
	}
	return mod.RenderAuxiliaryPrompt(n.suggesterPersona, suggesterEngineConstraints)
}
...
func (n *Narrator) SuggestActions(ctx context.Context, w model.World, players []role.Player, narrative string) (role.ActionChoices, error) {
	...
	messages := []*schema.Message{
		schema.SystemMessage(n.SuggesterPrompt()),
...
func buildSuggestPrompt(w model.World, players []role.Player, narrative string) string {
	...
	if active := view.ActiveThreads(w.Threads); len(active) > 0 {
		b.WriteString("\n## 活跃线索\n")
